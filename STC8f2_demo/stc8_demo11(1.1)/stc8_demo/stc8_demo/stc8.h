...
//例：sfr IE2=0xaf;把单片机的地址0xaf改名字为IE2
//?????????	
sfr IE          =   0xa8;						//中断使能寄存器1
sbit EA         =   IE^7; 					//CPU开放中断
sbit ELVD       =   IE^6;						//即定义ELVD为IE口的第7位，便于位操作;	
sbit EADC       =   IE^5;
sbit ES         =   IE^4;
sbit ET1        =   IE^3;
sbit EX1        =   IE^2;
sbit ET0        =   IE^1;
sbit EX0        =   IE^0; 					//即定义EX0为IE口的第1位，便于位操作;

sfr IE2         =   0xaf;						//中断使能寄存器2
 //sbit ET4        =   IE2^7;
#define ET4         0x40						//允许T4（计算器溢出）中断
#define ET3         0x20
#define ES4         0x10
#define ES3         0x08
#define ET2         0x04
...
sfr T3L         =   0xd5;
sfr T2H         =   0xd6;
sfr T2L         =   0xd7;     //定时器2计数器低8位
sfr TH4         =   0xd2;     //定时器2计数器高8位
sfr TL4         =   0xd3;
sfr TH3         =   0xd4;
sfr TL3         =   0xd5;
sfr TH2         =   0xd6;
...
//??????????
sfr SCON        =   0x98;		//串口1控制寄存器  该寄存器有8位,如下：
sbit SM0        =   SCON^7;
sbit SM1        =   SCON^6;
sbit SM2        =   SCON^5;
sbit REN        =   SCON^4;
...
sbit RB8        =   SCON^2;
sbit TI         =   SCON^1;
sbit RI         =   SCON^0;	//串口1接收完成中断标志 (需软件清零)
//1001 1000
//1001 1001
